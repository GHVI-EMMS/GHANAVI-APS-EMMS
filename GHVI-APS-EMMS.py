...
def sync_db_if_needed():
    """Sync database if connected to internet."""
    if is_internet_connected():
        print("Internet connection detected. Syncing database...")
        sync_database()
    else:
        print("No internet connection, using local database")


def resolve_conflicts(local_db_path, remote_db_path):
    """Resolve conflicts between local and remote database files."""
    try:
        # Create a backup of the local database
        backup_path = local_db_path + ".backup"
        shutil.copyfile(local_db_path, backup_path)

        # Merge changes from remote database into local database
        local_conn = sqlite3.connect(local_db_path)
        remote_conn = sqlite3.connect(remote_db_path)

        # Example: Copy all tables from remote to local
        remote_cursor = remote_conn.cursor()
        remote_cursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
        ...
        for table in tables:
            ...
            remote_cursor.execute(f"SELECT sql FROM sqlite_master WHERE type='table' AND name='{table_name}';")
            create_table_sql = remote_cursor.fetchone()[0]
            ...
            for row in rows:
                ...
                local_cursor.execute(f"INSERT INTO {table_name} VALUES ({placeholders});", row)
        ...
        messagebox.showerror("Conflict Resolution Error", f"Failed to resolve conflicts: {e}")


def sync_database_with_conflict_resolution():
    """Synchronize database with Google Drive, resolving conflicts if necessary."""
    try:
        print("Attempting to sync database with Google Drive...")
        service = get_google_drive_service()
...
def check_connectivity(light_canvas, label, root):
    ...
    root.after(5000, check_connectivity, light_canvas, label, root)


def sync_and_exit(root, connectivity_light, connectivity_label):
...
def setup_connectivity_light(root):
    ...
    sync_now_button = ttk.Button(connectivity_light_frame, text="Sync Now", command=sync_button_click)
    ...
    exit_button = ttk.Button(connectivity_light_frame, text="Exit",
                             command=lambda: sync_and_exit(root, connectivity_light, connectivity_label))
    exit_button.pack(pady=5)

    # Start periodic connectivity check
    check_connectivity(connectivity_light, connectivity_label, root)

    # Add the "Update" button
    # update_button = ttk.Button(connectivity_light_frame, text="Update", command=update_app)
    # update_button.pack(pady=5)

    # Start periodic connectivity check
    # check_connectivity(connectivity_light, connectivity_label, root)  # Corrected function call with label
...
def setup_auto_sync(interval_minutes=1):
    """Setup automatic database synchronization."""

    def sync_task():
        sync_db_if_needed_with_conflict_resolution()
        root.after(interval_minutes * 60 * 1000, sync_task)

    root.after(0, sync_task)
...
def sync_db_if_needed_with_conflict_resolution():
    """Sync database if connected to internet, resolving conflicts if necessary."""
    print("Attempting to sync database with Google Drive...")  # Debug statement
    if is_internet_connected():
        print("Internet connection detected. Syncing database...")  # Debug statement
        try:
            sync_database_with_conflict_resolution()
        except Exception as e:
            print(f"Error during sync: {e}")  # Debug statement
            messagebox.showerror("Sync Error", f"Failed to sync database: {e}")
    else:
        print("No internet connection, using local database")  # Debug statement
...
def launch_main_app(root):
    ...
    global member_tree  # Declare member_tree as a global variable
...
def refresh_member_table():
    global member_tree  # Access the global member_tree variable
